...
const ImagePlaceholder: React.FC<{ isGenerating?: boolean }> = ({ isGenerating }) => (
  <div className="w-full h-full bg-purple-100 rounded-2xl flex flex-col items-center justify-center gap-3 animate-pulse">
    ...
    {isGenerating && <p className="text-purple-400 text-sm font-medium">Painting the scene...</p>}
  </div>
);
      ...
      >
        <div className="flex-1 flex items-center justify-center">
          {page.isGenerating ? (
            ...
            <p className={`leading-relaxed text-gray-700 ${generateImages ? 'text-xl md:text-2xl' : 'text-2xl md:text-3xl text-center max-w-xl'}`}>
              {page.text}
            </p>
          )}
        </div>
